...
Run from anywhere — this script locates the repo root relative to its own path
(scripts/Build.py) and chdirs there before doing anything.

Usage:
    python scripts/Build.py                  # full build (binary + installer)
    python scripts/Build.py --skip-wheel     # reuse existing wheel in dist/
    python scripts/Build.py --rebuild-dist   # force re-install packages into distribution
    python scripts/Build.py --skip-installer # skip NSIS installer step
    python scripts/Build.py --installer-only # only build installer, reuse dist/ffl-mcp.exe
...
# This script lives at <repo root>/scripts/Build.py.
...
def _readDependency(packageName: str) -> str:
    projectPath = REPO_ROOT / "pyproject.toml"
...
def generateInstallScript() -> str:
    ...
    lines = [
        "# Auto-generated by scripts/Build.py",
...
def buildInstaller(exePath: Path) -> Optional[Path]:
    ...
    if nsisPath is None:
        ...
        print("  Then re-run:  python scripts/Build.py --installer-only")
        return None
